...
def getSQL_CI_23_急性心筋梗塞における主要手術_治療実施率_Kコード別(HOSPITAL_ID, Kcode_ID, _C):
    return  "SELECT  " + \
	            "CI_23" + _C + ".年度 ,  " + \
	            "CI_23" + _C + ".期,  " + \
                "CI_23" + _C + ".Kコード別実施率, " + \
            ...
            "FROM " + \
	            "CI_23" + _C + " " + \
            "WHERE " + \
	            "NOT CI_23" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_23" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_23" + _C + ".kCode_K_CODE_ID = " + str(Kcode_ID) +  \
            " ORDER BY CI_23" + _C + ".年度, CI_23" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_23 開始")
    if is_acute == "急性期":
        _C = ""
    else:
        _C = "_C"
    ...
    # 既存シートが存在する場合は削除
    if "CI_23" in wb.sheetnames:
        wb.remove(wb["CI_23"])
    wb.create_sheet("CI_23")
    sheet = wb["CI_23"]    
    ...
    #Kコードマスタの数だけループ
    for tempKcode in CONST.MASTA_K_CODE:

        #当該疾患の平均在院日数を取得
        #print(HOSPITAL_NAME + " " + tempDisease[1] + " " + tempSev[1])
        #print(HOSPITAL_NAME + " " + tempDisease[1] + " " + tempSev[1])
        #print(getSQL_CI_02_平均在院日数(HOSPITAL_ID, tempDisease[0], tempSev[0]))
        temp性別実施率 = X_01.excuteSQL(getSQL_CI_23_急性心筋梗塞における主要手術_治療実施率_Kコード別(HOSPITAL_ID, tempKcode[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "急性心筋梗塞における主要手術_治療実施率_Kコード別"
        # 3行目に疾患名
        sheet.cell(3, 1 + (colCnt - 1) * roopColCnt).value = tempKcode[1]
        # 4行名に重症度、年代別、性別
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "Kコード別実施率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "Kコード別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "Kコード別実施率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "Kコード別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "Kコード別実施率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "Kコード別症例数_比較用"
