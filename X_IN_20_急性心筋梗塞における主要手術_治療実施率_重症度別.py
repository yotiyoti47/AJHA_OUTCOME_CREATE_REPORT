...
def getSQL_CI_20_急性心筋梗塞における主要手術_治療実施率_重症度別(HOSPITAL_ID, Severity_ID, _C):
    return  "SELECT  " + \
	            "CI_20" + _C + ".年度 ,  " + \
	            "CI_20" + _C + ".期,  " + \
                "CI_20" + _C + ".重症度別実施率, " + \
            ...
            "FROM " + \
	            "CI_20" + _C + " " + \
            "WHERE " + \
	            "NOT CI_20" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_20" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_20" + _C + ".Severity_SEVERITY_ID = " + str(Severity_ID) +  \
            " ORDER BY CI_20" + _C + ".年度, CI_20" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(HOSPITAL_NAME + " CI_20 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_20" in wb.sheetnames:
        wb.remove(wb["CI_20"])
    wb.create_sheet("CI_20")
    sheet = wb["CI_20"]    
    ...
    for tempSev in sevList:
        ...
        temp重症度別実施率 = X_01.excuteSQL(getSQL_CI_20_急性心筋梗塞における主要手術_治療実施率_重症度別(HOSPITAL_ID, tempSev[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "急性心筋梗塞における主要手術_治療実施率_重症度別"
        # 3行目に疾患名
        sheet.cell(3, 1 + (colCnt - 1) * roopColCnt).value = tempSev[1]
        # 4行名に重症度、年代別、性別
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "重症度別実施率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "重症度別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "重症度別実施率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "重症度別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "重症度別実施率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "重症度別症例数_比較用"


        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp重症度別実施率:
...
def getRepAgeData_old(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    ...
    for tempSev in sevList:
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "重症度別実施率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "重症度別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "疾患別患者数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "重症度別実施率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "重症度別症例数_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "疾患別患者数_比較用"

        if temp重症度別実施率 is None:
            print(" " + HOSPITAL_NAME + " CI_20 データなし")
            return

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp重症度別実施率:
            #辞書から疾患別患者数を取り出すためのキー（疾患IDは5を使用）
            key = str(tempRow[0]) + str(tempRow[1]) + "5"
