...
def getSQL_CI_06_死亡率_疾患別_年代別(disease_ID, age_ID, _C):
    return  "SELECT  " + \
	            "CI_06" + _C + ".年度 ,  " + \
	            "CI_06" + _C + ".期,  " + \
                "CI_06" + _C + ".死亡率, " + \
            ...
            "FROM " + \
	            "CI_06" + _C + " " + \
            "WHERE " + \
	            "NOT CI_06" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_06" + _C + ".Disease_DISEASE_ID = " + str(disease_ID) + " AND " + \
                "CI_06" + _C + ".Age_AGE_ID = " + str(age_ID) +  \
            " ORDER BY CI_06" + _C + ".年度, CI_06" + _C + ".期,  CI_06" + _C + ".Disease_DISEASE_ID, CI_06" + _C + ".Age_AGE_ID " 
...
def getRepAgeData(wb, is_acute):
    ...
    # 既存シートが存在する場合は削除
    if "CI_06" in wb.sheetnames:
        wb.remove(wb["CI_06"])
    wb.create_sheet("CI_06")
    sheet = wb["CI_06"]    
   ...
   # 書込み列の繰り返し数
    ...
    for tempDisease in CONST.MASTA_DISEASE:
        ...
        for tempAge in CONST.MASTA_AGE:
            ...
            temp死亡率 = X_01.excuteSQL(getSQL_CI_06_死亡率_疾患別_年代別(tempDisease[0], tempAge[0], _C))
