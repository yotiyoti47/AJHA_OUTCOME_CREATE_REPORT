...
# 2024年度にCI番号変更
# CI_100　→　CI_69_1
# ただし、DBのテーブルは「CI_100」のまま

def getSQL_CI_69_1_ひと月あたり症例数_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_100" + _C + ".年度 ,  " + \
	            "CI_100" + _C + ".月, " + \
                "CI_100" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_100" + _C + " " + \
            "WHERE " + \
	            "CI_100" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_100" + _C + ".年度, CI_100" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_69_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_69_1" in wb.sheetnames:
        wb.remove(wb["CI_69_1"])
    wb.create_sheet("CI_69_1")
    sheet = wb["CI_69_1"]    

    #ひと月あたり症例数_回復リハ
    tempひと月あたり症例数_回復リハ = X_01.excuteSQL(getSQL_CI_69_1_ひと月あたり症例数_回復リハ(HOSPITAL_ID, _C))

    if tempひと月あたり症例数_回復リハ is None:
        print("エラー: ひと月あたり症例数_回復リハの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "ひと月あたり症例数_回復リハ"
    # 3行目に疾患名
...
# 6行目からデータ入力
    ...
    print(" " + HOSPITAL_NAME + " CI_69_1 終了")
