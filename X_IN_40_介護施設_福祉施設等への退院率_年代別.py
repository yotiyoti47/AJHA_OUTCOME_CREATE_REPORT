...
def getSQL_CI_40_介護施設_福祉施設等への退院率_年代別(HOSPITAL_ID, Age_ID, _C):
    return  "SELECT  " + \
	            "CI_40" + _C + ".年度 ,  " + \
	            "CI_40" + _C + ".期,  " + \
                "CI_40" + _C + ".年代別他施設等への退院率, " + \
                "CI_40" + _C + ".年代別症例数 " + \
            "FROM " + \
	            "CI_40" + _C + " " + \
            "WHERE " + \
	            "NOT CI_40" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_40" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_40" + _C + ".Age_AGE_ID = " + str(Age_ID) +  \
            " ORDER BY CI_40" + _C + ".年度, CI_40" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_40 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_40" in wb.sheetnames:
        wb.remove(wb["CI_40"])
    wb.create_sheet("CI_40")
    sheet = wb["CI_40"]    
    ...
    for tempAge in CONST.MASTA_AGE:
        ...
        temp年代別他施設等への退院率 = X_01.excuteSQL(getSQL_CI_40_介護施設_福祉施設等への退院率_年代別(HOSPITAL_ID, tempAge[0], _C))
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "年代別他施設等への退院率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "年代別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "年代別他施設等への退院率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "年代別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "年代別他施設等への退院率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "年代別症例数_比較用"

        if temp年代別他施設等への退院率 is None:
            print("エラー: 年代別他施設等への退院率の取得に失敗しました。")
            return -1

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp年代別他施設等への退院率:
