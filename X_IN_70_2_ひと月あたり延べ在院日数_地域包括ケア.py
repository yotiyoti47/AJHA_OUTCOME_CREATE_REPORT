...
# 2024年度にCI番号変更
# CI_104　→　CI_70_2
# ただし、DBのテーブルは「CI_104」のまま
def getSQL_CI_70_2_ひと月あたり延べ在院日数_地域包括ケア(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_104" + _C + ".年度 ,  " + \
	            "CI_104" + _C + ".月, " + \
                "CI_104" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_104" + _C + " " + \
            "WHERE " + \
	            "CI_104" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_104" + _C + ".年度, CI_104" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_70_2 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_70_2" in wb.sheetnames:
        wb.remove(wb["CI_70_2"])
    wb.create_sheet("CI_70_2")
    sheet = wb["CI_70_2"]    

    #ひと月あたり延べ在院日数_地域包括ケア
    tempひと月あたり延べ在院日数_地域包括ケア = X_01.excuteSQL(getSQL_CI_70_2_ひと月あたり延べ在院日数_地域包括ケア(HOSPITAL_ID, _C))

    if tempひと月あたり延べ在院日数_地域包括ケア is None:
        print("エラー: ひと月あたり延べ在院日数_地域包括ケアの取得に失敗しました。")
        return -1
    ...
    for a in tempひと月あたり延べ在院日数_地域包括ケア:
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "ひと月あたり延べ在院日数_地域包括ケア"
    # 3行目に疾患名
    ...
    for year in yearList:
        for key in keyList:
            ...
            print(" " + HOSPITAL_NAME + " CI_70_2 終了")
