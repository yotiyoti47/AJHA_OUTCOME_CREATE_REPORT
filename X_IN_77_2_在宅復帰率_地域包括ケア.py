...
# 2024年度にCI番号変更
# CI_123　→　CI_77_2
# ただし、DBのテーブルは「CI_123」のまま
def getSQL_CI_77_2_在宅復帰率_地域包括ケア(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_123" + _C + ".年度 ,  " + \
	            "CI_123" + _C + ".月, " + \
                "CI_123" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_123" + _C + " " + \
            "WHERE " + \
	            "CI_123" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_123" + _C + ".年度, CI_123" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_77_2 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_77_2" in wb.sheetnames:
        wb.remove(wb["CI_77_2"])
    wb.create_sheet("CI_77_2")
    sheet = wb["CI_77_2"]    

    #在宅復帰率_地域包括ケア
    temp在宅復帰率_地域包括ケア = X_01.excuteSQL(getSQL_CI_77_2_在宅復帰率_地域包括ケア(HOSPITAL_ID, _C))

    if temp在宅復帰率_地域包括ケア is None:
        print("エラー: 在宅復帰率_地域包括ケアの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "在宅復帰率_地域包括ケア"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_77_2 終了")
