...
# 2024年度にCI番号変更
# CI_108　→　CI_71_3
# ただし、DBのテーブルは「CI_108」のまま
def getSQL_CI_71_3_平均在院日数_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_108" + _C + ".年度 ,  " + \
	            "CI_108" + _C + ".月, " + \
                "CI_108" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_108" + _C + " " + \
            "WHERE " + \
	            "CI_108" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_108" + _C + ".年度, CI_108" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_71_3 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_71_3" in wb.sheetnames:
        wb.remove(wb["CI_71_3"])
    wb.create_sheet("CI_71_3")
    sheet = wb["CI_71_3"]    

    #平均在院日数_療養
    temp平均在院日数_療養 = X_01.excuteSQL(getSQL_CI_71_3_平均在院日数_療養(HOSPITAL_ID, _C))

    if temp平均在院日数_療養 is None:
        print("エラー: 平均在院日数_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "平均在院日数_療養"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_71_3 終了")
