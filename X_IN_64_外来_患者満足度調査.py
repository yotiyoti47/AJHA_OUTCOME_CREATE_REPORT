...
# 2024年度にCI番号変更
# CI_83　→　CI_64
# ただし、DBのテーブルは「CI_83」のまま
def getSQL_CI_64_外来_患者満足度調査_回答率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_83" + _C + ".年度 ,  " + \
	            "CI_83" + _C + ".期,  " + \
                "CI_83" + _C + ".回答率, " + \
            ...
            "FROM " + \
	            "CI_83" + _C + " " + \
            "WHERE " + \
	            "NOT CI_83" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_83" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_83" + _C + ".年度, CI_83" + _C + ".期 " 

def getSQL_CI_64_外来_患者満足度調査_満足度_割合(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_83" + _C + ".年度 ,  " + \
	            "CI_83" + _C + ".期,  " + \
                "CI_83" + _C + ".満足_割合, " + \
...
def getSQL_CI_64_外来_患者満足度調査_満足度_件数(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_83" + _C + ".年度 ,  " + \
	            "CI_83" + _C + ".期,  " + \
                "CI_83" + _C + ".満足, " + \
...
def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    ...
    # 既存シートが存在する場合は削除
    if "CI_64" in wb.sheetnames:
        wb.remove(wb["CI_64"])
    wb.create_sheet("CI_64")
    sheet = wb["CI_64"]    
    ...
    #外来_患者満足度調査_回答率
    temp外来_患者満足度調査_回答率 = X_01.excuteSQL(getSQL_CI_64_外来_患者満足度調査_回答率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "外来_患者満足度調査_回答率"
    # 3行目に疾患名
    ...
    if temp外来_患者満足度調査_回答率 is None:
        print("エラー: 外来_患者満足度調査_回答率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp外来_患者満足度調査_回答率:
    ...
    #getSQL_CI_83_外来_患者満足度調査_満足度_割合
    temp外来_患者満足度調査_満足度_割合 = X_01.excuteSQL(getSQL_CI_64_外来_患者満足度調査_満足度_割合(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "外来_患者満足度調査_満足度_割合"
    # 3行目に疾患名
    sheet.cell(3, roopColCnt).value = ""
    # 4行名に重症度、年代別、性別
    sheet.cell(4, roopColCnt).value = ""
    ...
    if temp外来_患者満足度調査_満足度_割合 is None:
        print("エラー: 外来_患者満足度調査_満足度_割合の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp外来_患者満足度調査_満足度_割合:

        #年度
        sheet.cell(rowCnt, 13).value = tempRow[0]
        #期
        sheet.cell(rowCnt, 13 + 1).value = tempRow[1]
        
        #満足_割合
        if tempRow[2] == -1 or tempRow[2] == -2:
            sheet.cell(rowCnt, 13 + 2).value = 0
        else:
            sheet.cell(rowCnt, 13 + 2).value = tempRow[2] 
        #やや満足_割合
        if tempRow[3] == -1 or tempRow[3] == -2:
            sheet.cell(rowCnt, 13 + 3).value = 0
        else:
            sheet.cell(rowCnt, 13 + 3).value = tempRow[3] 
        #やや不満_割合
        if tempRow[4] == -1 or tempRow[4] == -2:
            sheet.cell(rowCnt, 13 + 4).value = 0
        else:
            sheet.cell(rowCnt, 13 + 4).value = tempRow[4] 
        #不満_割合
        if tempRow[5] == -1 or tempRow[5] == -2:
            sheet.cell(rowCnt, 13 + 5).value = 0
        else:
            sheet.cell(rowCnt, 13 + 5).value = tempRow[5] 
        #無効回答_割合
        if tempRow[6] == -1 or tempRow[6] == -2:
            sheet.cell(rowCnt, 13 + 6).value = 0
        else:
            sheet.cell(rowCnt, 13 + 6).value = tempRow[6] 

        #満足_割合_ラベル
        if tempRow[2] == -1:
            sheet.cell(rowCnt,  13 + 7).value = "N/A"
        elif tempRow[2] == -2:
            sheet.cell(rowCnt,  13 + 7).value = "-"
        else:
            sheet.cell(rowCnt,  13 + 7).value = tempRow[2]
        #やや満足_割合_ラベル
        if tempRow[3] == -1:
            sheet.cell(rowCnt,  13 + 8).value = "N/A"
        elif tempRow[3] == -2:
            sheet.cell(rowCnt,  13 + 8).value = "-"
        else:
            sheet.cell(rowCnt,  13 + 8).value = tempRow[3]
        #やや不満_割合_ラベル
        if tempRow[4] == -1:
            sheet.cell(rowCnt,  13 + 9).value = "N/A"
        elif tempRow[4] == -2:
            sheet.cell(rowCnt,  13 + 9).value = "-"
        else:
            sheet.cell(rowCnt,  13 + 9).value = tempRow[4]
        #不満_割合_ラベル
        if tempRow[5] == -1:
            sheet.cell(rowCnt,  13 + 10).value = "N/A"
        elif tempRow[5] == -2:
            sheet.cell(rowCnt,  13 + 10).value = "-"
        else:
            sheet.cell(rowCnt,  13 + 10).value = tempRow[5]
        #無効回答_割合_ラベル
        if tempRow[6] == -1:
            sheet.cell(rowCnt,  13 + 11).value = "N/A"
        elif tempRow[6] == -2:
            sheet.cell(rowCnt,  13 + 11).value = "-"
        else:
            sheet.cell(rowCnt,  13 + 11).value = tempRow[6]

        #満足_割合_比較用
        sheet.cell(rowCnt,  13 + 12).value = tempRow[2]
        #やや満足_割合_比較用
        sheet.cell(rowCnt,  13 + 13).value = tempRow[3]
        #やや不満_割合_比較用
        sheet.cell(rowCnt,  13 + 14).value = tempRow[4]
        #不満_割合_比較用
        sheet.cell(rowCnt,  13 + 15).value = tempRow[5]
        #無効回答_割合_比較用
        sheet.cell(rowCnt,  13 + 16).value = tempRow[6]
    ...
    #外来_患者満足度調査_満足度_件数
    temp外来_患者満足度調査_満足度_件数 = X_01.excuteSQL(getSQL_CI_64_外来_患者満足度調査_満足度_件数(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "外来_患者満足度調査_満足度_件数"
    # 3行目に疾患名
    sheet.cell(3, roopColCnt).value = ""
    # 4行名に重症度、年代別、性別
    sheet.cell(4, roopColCnt).value = ""
    ...
    if temp外来_患者満足度調査_満足度_件数 is None:
        print("エラー: 外来_患者満足度調査_満足度_件数の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp外来_患者満足度調査_満足度_件数:
