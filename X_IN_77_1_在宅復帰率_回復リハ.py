...
# 2024年度にCI番号変更
# CI_122　→　CI_77_1
# ただし、DBのテーブルは「CI_122」のまま
def getSQL_CI_77_1_在宅復帰率_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_122" + _C + ".年度 ,  " + \
	            "CI_122" + _C + ".月, " + \
                "CI_122" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_122" + _C + " " + \
            "WHERE " + \
	            "CI_122" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_122" + _C + ".年度, CI_122" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_77_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_77_1" in wb.sheetnames:
        wb.remove(wb["CI_77_1"])
    wb.create_sheet("CI_77_1")
    sheet = wb["CI_77_1"]    

    #在宅復帰率_回復リハ
    temp在宅復帰率_回復リハ = X_01.excuteSQL(getSQL_CI_77_1_在宅復帰率_回復リハ(HOSPITAL_ID, _C))

    if temp在宅復帰率_回復リハ is None:
        print("エラー: 在宅復帰率_回復リハの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "在宅復帰率_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_77_1 終了")
