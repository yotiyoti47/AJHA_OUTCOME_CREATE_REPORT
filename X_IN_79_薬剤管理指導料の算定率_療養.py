...
# 2024年度にCI番号変更
# CI_126　→　CI_79
# ただし、DBのテーブルは「CI_126」のまま
def getSQL_CI_79_薬剤管理指導料の算定率_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_126" + _C + ".年度 ,  " + \
	            "CI_126" + _C + ".月, " + \
                "CI_126" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_126" + _C + " " + \
            "WHERE " + \
	            "CI_126" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_126" + _C + ".年度, CI_126" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_79 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_79" in wb.sheetnames:
        wb.remove(wb["CI_79"])
    wb.create_sheet("CI_79")
    sheet = wb["CI_79"]    

    #薬剤管理指導料の算定率_療養
    temp薬剤管理指導料の算定率_療養 = X_01.excuteSQL(getSQL_CI_79_薬剤管理指導料の算定率_療養(HOSPITAL_ID, _C))

    if temp薬剤管理指導料の算定率_療養 is None:
        print("エラー: 薬剤管理指導料の算定率_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "薬剤管理指導料の算定率_療養"
    # 3行目に疾患名
