...
# 2024年度にCI番号変更
# CI_121　→　CI_76_3
# ただし、DBのテーブルは「CI_121」のまま
def getSQL_CI_76_3_紹介率_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_121" + _C + ".年度 ,  " + \
	            "CI_121" + _C + ".月, " + \
                "CI_121" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_121" + _C + " " + \
            "WHERE " + \
	            "CI_121" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_121" + _C + ".年度, CI_121" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_76_3 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_76_3" in wb.sheetnames:
        wb.remove(wb["CI_76_3"])
    wb.create_sheet("CI_76_3")
    sheet = wb["CI_76_3"]    

    #紹介率_療養
    temp紹介率_療養 = X_01.excuteSQL(getSQL_CI_76_3_紹介率_療養(HOSPITAL_ID, _C))

    if temp紹介率_療養 is None:
        print("エラー: 紹介率_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "紹介率_療養"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_76_3 終了")
