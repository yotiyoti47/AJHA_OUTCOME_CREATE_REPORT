...
def getSQL_CI_11_再入院率_疾患別_性別(HOSPITAL_ID, disease_ID, Sex_ID, _C):
    return  "SELECT  " + \
	            "CI_11" + _C + ".年度 ,  " + \
	            "CI_11" + _C + ".期,  " + \
                "CI_11" + _C + ".再入院率 " + \
            "FROM " + \
	            "CI_11" + _C + " " + \
            "WHERE " + \
	            "NOT CI_11" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_11" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_11" + _C + ".Disease_DISEASE_ID = " + str(disease_ID) + " AND " + \
                "CI_11" + _C + ".Sex_SEX_ID = " + str(Sex_ID) +  \
            " ORDER BY CI_11" + _C + ".年度, CI_11" + _C + ".期 " 


def getRepAgeData_性別(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_11 開始")
    ...
    for tempDisease in CONST.MASTA_DISEASE:
        ...
        for tempSex in CONST.MASTA_SEX:
            ...
            temp再入院率 = X_01.excuteSQL(getSQL_CI_11_再入院率_疾患別_性別(HOSPITAL_ID, tempDisease[0], tempSex[0], _C))
            dic_再入院率[str(tempDisease[0]) + str(tempSex[0])] = temp再入院率
    ...
    for tempDisease in CONST.MASTA_DISEASE:
        ...
        for i in range(3):
            if i == 0:  
                ...
                # 2行目に指標名
                sheet.cell(2, targetColCnt).value = "予定しない再入院率_疾患別_性別"
                # 3行目に疾患名
            ...
            for tempSex in CONST.MASTA_SEX:
                #辞書から平均在院日数リストを取得
                tempList = dic_再入院率[str(tempDisease[0]) + str(tempSex[0])]
