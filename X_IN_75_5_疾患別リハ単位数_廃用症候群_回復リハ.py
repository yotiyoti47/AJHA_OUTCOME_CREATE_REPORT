...
# 2024年度にCI番号変更
# CI_118　→　CI_75_5
# ただし、DBのテーブルは「CI_118」のまま
def getSQL_CI_75_5_疾患別リハ単位数_廃用症候群_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_118" + _C + ".年度 ,  " + \
	            "CI_118" + _C + ".月, " + \
                "CI_118" + _C + ".病床数, " + \
                "CI_118" + _C + "._1症例あたり単位数, " + \
                "CI_118" + _C + ".合計単位数, " + \
                "CI_118" + _C + ".症例数 " + \
            "FROM " + \
	            "CI_118" + _C + " " + \
            "WHERE " + \
	            "CI_118" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_118" + _C + ".年度, CI_118" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_75_5 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_75_5" in wb.sheetnames:
        wb.remove(wb["CI_75_5"])
    wb.create_sheet("CI_75_5")
    sheet = wb["CI_75_5"]    

    #疾患別リハ単位数_廃用症候群_回復リハ
    temp疾患別リハ単位数_廃用症候群_回復リハ = X_01.excuteSQL(getSQL_CI_75_5_疾患別リハ単位数_廃用症候群_回復リハ(HOSPITAL_ID, _C))

    if temp疾患別リハ単位数_廃用症候群_回復リハ is None:
        print("エラー: 疾患別リハ単位数_廃用症候群_回復リハの取得に失敗しました。")
        return -1
    ...
    for a in temp疾患別リハ単位数_廃用症候群_回復リハ:
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "疾患別リハ単位数_廃用症候群_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_75_5 終了")
