...
def getSQL_CI_36_大腿骨地域連携パスの使用率_転院先(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_36" + _C + ".年度 ,  " + \
	            "CI_36" + _C + ".期,  " + \
                "CI_36" + _C + ".大腿骨頸部骨折連携パスの使用率_転院先, " + \
                "CI_36" + _C + ".うち_地域連携診療計画退院時指導料_１_算定症例数, " + \
                "CI_36" + _C + ".大腿骨頸部骨折が医資最病名である退院症例数 " + \
            "FROM " + \
	            "CI_36" + _C + " " + \
            "WHERE " + \
	            "NOT CI_36" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_36" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_36" + _C + ".年度, CI_36" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_36 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_36" in wb.sheetnames:
        wb.remove(wb["CI_36"])
    wb.create_sheet("CI_36")
    sheet = wb["CI_36"]    
    ...
    #大腿骨頸部骨折連携パスの使用率_転院先
    temp大腿骨頸部骨折連携パスの使用率_転院先 = X_01.excuteSQL(getSQL_CI_36_大腿骨地域連携パスの使用率_転院先(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "大腿骨頸部骨折連携パスの使用率_転院先"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "大腿骨頸部骨折が医資最病名である退院症例数"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "大腿骨頸部骨折が医資最病名である退院症例数_ラベル"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "大腿骨頸部骨折が医資最病名である退院症例数_比較用"

    if temp大腿骨頸部骨折連携パスの使用率_転院先 is None:
        print("エラー: 大腿骨頸部骨折連携パスの使用率_転院先の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp大腿骨頸部骨折連携パスの使用率_転院先:
