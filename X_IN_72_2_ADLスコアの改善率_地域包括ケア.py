...
# 2024年度にCI番号変更
# CI_110　→　CI_72_2
# ただし、DBのテーブルは「CI_110」のまま
def getSQL_CI_72_2_ADLスコアの改善率_地域包括ケア(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_110" + _C + ".年度 ,  " + \
	            "CI_110" + _C + ".月, " + \
                "CI_110" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_110" + _C + " " + \
            "WHERE " + \
	            "CI_110" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_110" + _C + ".年度, CI_110" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_72_2 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_72_2" in wb.sheetnames:
        wb.remove(wb["CI_72_2"])
    wb.create_sheet("CI_72_2")
    sheet = wb["CI_72_2"]    

    #ADLスコアの改善率_地域包括ケア
    tempADLスコアの改善率_地域包括ケア = X_01.excuteSQL(getSQL_CI_72_2_ADLスコアの改善率_地域包括ケア(HOSPITAL_ID, _C))

    if tempADLスコアの改善率_地域包括ケア is None:
        print("エラー: ADLスコアの改善率_地域包括ケアの取得に失敗しました。")
        return -1
    ...
    for a in tempADLスコアの改善率_地域包括ケア:
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "ADLスコアの改善率_地域包括ケア"
    # 3行目に疾患名
    ...
    for year in yearList:
        for key in keyList:
            ...
            rowCnt += 1
    print(" " + HOSPITAL_NAME + " CI_72_2 終了")
