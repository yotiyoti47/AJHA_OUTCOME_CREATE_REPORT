...
# 2024年度にCI番号変更
# CI_113　→　CI_74
# ただし、DBのテーブルは「CI_113」のまま
def getSQL_CI_74_医療区分の改善率_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_113" + _C + ".年度 ,  " + \
	            "CI_113" + _C + ".月, " + \
                "CI_113" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_113" + _C + " " + \
            "WHERE " + \
	            "CI_113" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_113" + _C + ".年度, CI_113" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_74 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_74" in wb.sheetnames:
        wb.remove(wb["CI_74"])
    wb.create_sheet("CI_74")
    sheet = wb["CI_74"]    

    #医療区分の改善率_療養
    temp医療区分の改善率_療養 = X_01.excuteSQL(getSQL_CI_74_医療区分の改善率_療養(HOSPITAL_ID, _C))

    if temp医療区分の改善率_療養 is None:
        print("エラー: 医療区分の改善率_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "医療区分の改善率_療養"
    # 3行目に疾患名
