...
# 2024年度にCI番号変更
# CI_80　→　CI_61
# ただし、DBのテーブルは「CI_80」のまま
def getSQL_CI_61_入院_患者満足度調査_回答率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_80" + _C + ".年度 ,  " + \
	            "CI_80" + _C + ".期,  " + \
                "CI_80" + _C + ".回答率, " + \
            ...
            "FROM " + \
	            "CI_80" + _C + " " + \
            "WHERE " + \
	            "NOT CI_80" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_80" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_80" + _C + ".年度, CI_80" + _C + ".期 " 

def getSQL_CI_61_入院_患者満足度調査_満足度_割合(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_80" + _C + ".年度 ,  " + \
	            "CI_80" + _C + ".期,  " + \
                "CI_80" + _C + ".満足_割合, " + \
...
def getSQL_CI_61_入院_患者満足度調査_満足度_件数(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_80" + _C + ".年度 ,  " + \
	            "CI_80" + _C + ".期,  " + \
                "CI_80" + _C + ".満足, " + \
...
def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    ...
    # 既存シートが存在する場合は削除
    if "CI_61" in wb.sheetnames:
        wb.remove(wb["CI_61"])
    wb.create_sheet("CI_61")
    sheet = wb["CI_61"]    
    ...
    #入院_患者満足度調査_回答率
    temp入院_患者満足度調査_回答率 = X_01.excuteSQL(getSQL_CI_61_入院_患者満足度調査_回答率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1).value = "入院_患者満足度調査_回答率"
    # 3行目に疾患名
    ...
    if temp入院_患者満足度調査_回答率 is None:  
        print("エラー: 入院_患者満足度調査_回答率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp入院_患者満足度調査_回答率:
    ...
    #getSQL_CI_80_入院_患者満足度調査_満足度_割合
    temp入院_患者満足度調査_満足度_割合 = X_01.excuteSQL(getSQL_CI_61_入院_患者満足度調査_満足度_割合(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "入院_患者満足度調査_満足度_割合"
    # 3行目に疾患名
    ...
    if temp入院_患者満足度調査_満足度_割合 is None:
        print("エラー: 入院_患者満足度調査_満足度_割合の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp入院_患者満足度調査_満足度_割合:
    ...
    #入院_患者満足度調査_満足度_件数
    temp入院_患者満足度調査_満足度_件数 = X_01.excuteSQL(getSQL_CI_61_入院_患者満足度調査_満足度_件数(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "入院_患者満足度調査_満足度_件数"
    # 3行目に疾患名
    sheet.cell(3, roopColCnt).value = ""
    # 4行名に重症度、年代別、性別
    sheet.cell(4, roopColCnt).value = ""
    ...
    if temp入院_患者満足度調査_満足度_件数 is None:
        print("エラー: 入院_患者満足度調査_満足度_件数の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp入院_患者満足度調査_満足度_件数:
