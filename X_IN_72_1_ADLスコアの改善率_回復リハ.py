...
# 2024年度にCI番号変更
# CI_109　→　CI_72_1
# ただし、DBのテーブルは「CI_109」のまま
def getSQL_CI_72_1_ADLスコアの改善率_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_109" + _C + ".年度 ,  " + \
	            "CI_109" + _C + ".月, " + \
                "CI_109" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_109" + _C + " " + \
            "WHERE " + \
	            "CI_109" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_109" + _C + ".年度, CI_109" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_72_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_72_1" in wb.sheetnames:
        wb.remove(wb["CI_72_1"])
    wb.create_sheet("CI_72_1")
    sheet = wb["CI_72_1"]    

    #ADLスコアの改善率_回復リハ
    tempADLスコアの改善率_回復リハ = X_01.excuteSQL(getSQL_CI_72_1_ADLスコアの改善率_回復リハ(HOSPITAL_ID, _C))

    if tempADLスコアの改善率_回復リハ is None:
        print("エラー: ADLスコアの改善率_回復リハの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "ADLスコアの改善率_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_72_1 終了")
