...
def getSQL_CI_08_予定しない再入院率(HOSPITAL_ID, disease_ID, _C):
    return  "SELECT  " + \
	            "CI_08" + _C + ".年度 ,  " + \
	            "CI_08" + _C + ".期,  " + \
                "CI_08" + _C + ".再入院率 " + \
            "FROM " + \
	            "CI_08" + _C + " " + \
            "WHERE " + \
	            "NOT CI_08" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_08" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_08" + _C + ".Disease_DISEASE_ID = " + str(disease_ID) + \
            " ORDER BY CI_08" + _C + ".年度, CI_08" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_08 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_08" in wb.sheetnames:
        wb.remove(wb["CI_08"])
    wb.create_sheet("CI_08")
    sheet = wb["CI_08"]    
    ...
    try:
        ...
        for tempDisease in CONST.MASTA_DISEASE:
            #当該疾患の再入院率を取得
            temp再入院率 = X_01.excuteSQL(getSQL_CI_08_予定しない再入院率(HOSPITAL_ID, tempDisease[0], _C))
            if temp再入院率 is None:
                print("エラー: 再入院率の取得に失敗しました。")
                return -1
            ...
            # 2行目に指標名
            sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "予定しない再入院率_疾患別"
            # 3行目に疾患名
            ...
            sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "予定しない再入院率"
            sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "疾患別患者数"
            sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "予定しない再入院率_ラベル"
            sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "疾患別患者数_ラベル"
            sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "予定しない再入院率_比較用"
            sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "疾患別患者数_比較用"

            # 6行目からデータ入力
            rowCnt = 6
            for tempRow in temp再入院率:
            ...
            colCnt+=1
        #wb.save(wbPath)
        print(" " + HOSPITAL_NAME + " CI_08 終了")
    except Exception as e:
        print("エラー: CI_08の処理で例外が発生しました。")
        print(e)
        return -1
