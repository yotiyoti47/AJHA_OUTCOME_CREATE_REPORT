...
def getSQL_CI_52_服薬指導_薬剤管理指導実施率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_52" + _C + ".年度 ,  " + \
	            "CI_52" + _C + ".期,  " + \
                "CI_52" + _C + ".服薬指導_薬剤管理指導実施率, " + \
                "CI_52" + _C + ".分母のうち薬剤管理指導を受けた患者数, " + \
                "CI_52" + _C + ".入院患者数 " + \
            "FROM " + \
	            "CI_52" + _C + " " + \
            "WHERE " + \
	            "NOT CI_52" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_52" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_52" + _C + ".年度, CI_52" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_52 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_52" in wb.sheetnames:
        wb.remove(wb["CI_52"])
    wb.create_sheet("CI_52")
    sheet = wb["CI_52"]    
    ...
    #服薬指導_薬剤管理指導実施率
    temp服薬指導_薬剤管理指導実施率 = X_01.excuteSQL(getSQL_CI_52_服薬指導_薬剤管理指導実施率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "服薬指導_薬剤管理指導実施率"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "服薬指導_薬剤管理指導実施率"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "分母のうち薬剤管理指導を受けた患者数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "退院患者数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "服薬指導_薬剤管理指導実施率_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "分母のうち薬剤管理指導を受けた患者数_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "退院患者数_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "服薬指導_薬剤管理指導実施率_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "分母のうち薬剤管理指導を受けた患者数_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "退院患者数_比較用"

    if temp服薬指導_薬剤管理指導実施率 is None:
        print("エラー: 服薬指導_薬剤管理指導実施率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp服薬指導_薬剤管理指導実施率:
