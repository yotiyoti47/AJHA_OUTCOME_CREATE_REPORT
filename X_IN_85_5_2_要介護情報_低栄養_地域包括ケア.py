...
# 2024年度にCI番号変更
# CI_148　→　CI_85-5_2
# ただし、DBのテーブルは「CI_148」のまま
def getSQL_CI_85_5_2_要介護情報_低栄養_地域包括ケア(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_148" + _C + ".年度 ,  " + \
	            "CI_148" + _C + ".月, " + \
                "CI_148" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_148" + _C + " " + \
            "WHERE " + \
	            "CI_148" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_148" + _C + ".年度, CI_148" + _C + ".月 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_85_5_2 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_85_5_2" in wb.sheetnames:
        wb.remove(wb["CI_85_5_2"])
    wb.create_sheet("CI_85_5_2")
    sheet = wb["CI_85_5_2"]    

    #要介護情報_低栄養_地域包括ケア
    temp要介護情報_低栄養_地域包括ケア = X_01.excuteSQL(getSQL_CI_85_5_2_要介護情報_低栄養_地域包括ケア(HOSPITAL_ID, _C))

    if temp要介護情報_低栄養_地域包括ケア is None:
        print("エラー: 要介護情報_低栄養_地域包括ケアの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "要介護情報_低栄養_地域包括ケア"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_85_5_2 終了")
