...
def getSQL_CI_39_他の病院_診療所の病棟からの転院率_年代別(HOSPITAL_ID, Age_ID, _C):
    return  "SELECT  " + \
	            "CI_39" + _C + ".年度 ,  " + \
	            "CI_39" + _C + ".期,  " + \
                "CI_39" + _C + ".年代別他病院からの転院率, " + \
                "CI_39" + _C + ".年代別症例数 " + \
            "FROM " + \
	            "CI_39" + _C + " " + \
            "WHERE " + \
	            "NOT CI_39" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_39" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_39" + _C + ".Age_AGE_ID = " + str(Age_ID) +  \
            " ORDER BY CI_39" + _C + ".年度, CI_39" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_39 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_39" in wb.sheetnames:
        wb.remove(wb["CI_39"])
    wb.create_sheet("CI_39")
    sheet = wb["CI_39"]    
    ...
    for tempAge in CONST.MASTA_AGE:
        ...
        temp年代別他病院からの転院率 = X_01.excuteSQL(getSQL_CI_39_他の病院_診療所の病棟からの転院率_年代別(HOSPITAL_ID, tempAge[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "他の病院_診療所の病棟からの転院率_年代別"
        # 3行目に疾患名
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "年代別他病院からの転院率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "年代別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "年代別他病院からの転院率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "年代別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "年代別他病院からの転院率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "年代別症例数_比較用"

        if temp年代別他病院からの転院率 is None:
            print("エラー: 年代別他病院からの転院率の取得に失敗しました。")
            return -1

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp年代別他病院からの転院率:
