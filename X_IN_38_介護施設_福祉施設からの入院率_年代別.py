...
def getSQL_CI_38_介護施設_福祉施設からの入院率(HOSPITAL_ID, Age_ID, _C):
    return  "SELECT  " + \
	            "CI_38" + _C + ".年度 ,  " + \
	            "CI_38" + _C + ".期,  " + \
                "CI_38" + _C + ".年代別介護_福祉施設からの入院率, " + \
                "CI_38" + _C + ".年代別症例数 " + \
            "FROM " + \
	            "CI_38" + _C + " " + \
            "WHERE " + \
	            "NOT CI_38" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_38" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_38" + _C + ".Age_AGE_ID = " + str(Age_ID) +  \
            " ORDER BY CI_38" + _C + ".年度, CI_38" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_38 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_38" in wb.sheetnames:
        wb.remove(wb["CI_38"])
    wb.create_sheet("CI_38")
    sheet = wb["CI_38"]    
    ...
    for tempAge in CONST.MASTA_AGE:
        ...
        temp年代別介護_福祉施設からの入院率 = X_01.excuteSQL(getSQL_CI_38_介護施設_福祉施設からの入院率(HOSPITAL_ID, tempAge[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "介護施設_福祉施設からの入院率"
        # 3行目に疾患名
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "年代別介護_福祉施設からの入院率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "年代別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "年代別介護_福祉施設からの入院率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "年代別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "年代別介護_福祉施設からの入院率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "年代別症例数_比較用"

        if temp年代別介護_福祉施設からの入院率 is None:
            print("エラー: 年代別介護_福祉施設からの入院率の取得に失敗しました。")
            return -1

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp年代別介護_福祉施設からの入院率:
