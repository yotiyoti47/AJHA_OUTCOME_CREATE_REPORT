...
# 2024年度にCI番号変更
# CI_128　→　CI_81_1
# ただし、DBのテーブルは「CI_128」のまま
def getSQL_CI_81_1_目標設定等支援_管理料の算定率_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_128" + _C + ".年度 ,  " + \
	            "CI_128" + _C + ".月, " + \
                "CI_128" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_128" + _C + " " + \
            "WHERE " + \
	            "CI_128" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_128" + _C + ".年度, CI_128" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_81_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_81_1" in wb.sheetnames:
        wb.remove(wb["CI_81_1"])
    wb.create_sheet("CI_81_1")
    sheet = wb["CI_81_1"]    

    #目標設定等支援_管理料の算定率_回復リハ
    temp目標設定等支援_管理料の算定率_回復リハ = X_01.excuteSQL(getSQL_CI_81_1_目標設定等支援_管理料の算定率_回復リハ(HOSPITAL_ID, _C))

    if temp目標設定等支援_管理料の算定率_回復リハ is None:
        print("エラー: 目標設定等支援_管理料の算定率_回復リハの取得に失敗しました。")
        return -1
    ...
    for a in temp目標設定等支援_管理料の算定率_回復リハ:
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "目標設定等支援_管理料の算定率_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_81_1 終了")
