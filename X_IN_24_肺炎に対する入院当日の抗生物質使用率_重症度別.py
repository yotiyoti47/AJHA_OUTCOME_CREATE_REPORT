...
def getSQL_CI_24_肺炎に対する入院当日の抗生物質使用率_重症度別(HOSPITAL_ID, Severity_ID, _C):
    return  "SELECT  " + \
	            "CI_24" + _C + ".年度 ,  " + \
	            "CI_24" + _C + ".期,  " + \
                "CI_24" + _C + ".重症度別使用率, " + \
            ...
            "FROM " + \
	            "CI_24" + _C + " " + \
            "WHERE " + \
	            "NOT CI_24" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_24" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_24" + _C + ".Severity_SEVERITY_ID = " + str(Severity_ID) +  \
            " ORDER BY CI_24" + _C + ".年度, CI_24" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_24 開始")
    if is_acute == "急性期":
        _C = ""
    else:
        _C = "_C"
    ...
    # 既存シートが存在する場合は削除
    if "CI_24" in wb.sheetnames:
        wb.remove(wb["CI_24"])
    wb.create_sheet("CI_24")
    sheet = wb["CI_24"]    
    ...
    for tempSev in sevList:
        ...
        temp重症度別使用率 = X_01.excuteSQL(getSQL_CI_24_肺炎に対する入院当日の抗生物質使用率_重症度別(HOSPITAL_ID, tempSev[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "肺炎に対する入院当日の抗生物質使用率_重症度別"
        # 3行目に疾患名
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "重症度別使用率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "重症度別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "重症度別使用率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "重症度別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "重症度別使用率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "重症度別症例数_比較用"

        if temp重症度別使用率 is None:
            print("エラー: 重症度別使用率の取得に失敗しました。")
            return -1

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp重症度別使用率:
