...
# 2024年度にCI番号変更
# CI_140　→　CI_85-2_3
# ただし、DBのテーブルは「CI_140」のまま
def getSQL_CI_85_2_3_要介護情報_経鼻胃管_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_140" + _C + ".年度 ,  " + \
	            "CI_140" + _C + ".月, " + \
                "CI_140" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_140" + _C + " " + \
            "WHERE " + \
	            "CI_140" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_140" + _C + ".年度, CI_140" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_85_2_3 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_85_2_3" in wb.sheetnames:
        wb.remove(wb["CI_85_2_3"])
    wb.create_sheet("CI_85_2_3")
    sheet = wb["CI_85_2_3"]    

    #要介護情報_経鼻胃管_療養
    temp要介護情報_経鼻胃管_療養 = X_01.excuteSQL(getSQL_CI_85_2_3_要介護情報_経鼻胃管_療養(HOSPITAL_ID, _C))

    if temp要介護情報_経鼻胃管_療養 is None:
        print("エラー: 要介護情報_経鼻胃管_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "要介護情報_経鼻胃管_療養"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_85_2_3 終了")
