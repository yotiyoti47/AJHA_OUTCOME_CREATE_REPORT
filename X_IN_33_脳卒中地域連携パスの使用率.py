...
def getSQL_CI_33_脳卒中地域連携パスの使用率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_33" + _C + ".年度 ,  " + \
	            "CI_33" + _C + ".期,  " + \
                "CI_33" + _C + ".脳卒中地域連携パス使用率, " + \
                "CI_33" + _C + ".うち_地域連携診療計画退院時指導料_１_算定症例数, " + \
                "CI_33" + _C + ".脳卒中が医資最病名である退院症例数 " + \
            "FROM " + \
	            "CI_33" + _C + " " + \
            "WHERE " + \
	            "NOT CI_33" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_33" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_33" + _C + ".年度, CI_33" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_33 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_33" in wb.sheetnames:
        wb.remove(wb["CI_33"])
    wb.create_sheet("CI_33")
    sheet = wb["CI_33"]    
    ...
    #脳卒中地域連携パス使用率
    temp脳卒中地域連携パス使用率 = X_01.excuteSQL(getSQL_CI_33_脳卒中地域連携パスの使用率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "脳卒中地域連携パス使用率"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "脳卒中地域連携パス使用率"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "脳卒中地域連携パス使用率_ラベル"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "脳卒中地域連携パス使用率_比較用"
    ...
    if temp脳卒中地域連携パス使用率 is None:
        print("エラー: 脳卒中地域連携パス使用率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp脳卒中地域連携パス使用率:
