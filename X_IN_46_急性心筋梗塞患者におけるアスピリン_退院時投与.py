...
def getSQL_CI_46_急性心筋梗塞患者におけるアスピリン_退院時投与(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_46" + _C + ".年度 ,  " + \
	            "CI_46" + _C + ".期,  " + \
                "CI_46" + _C + ".急性心筋梗塞患者におけるアスピリン退院時投与率, " + \
                "CI_46" + _C + ".分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数, " + \
                "CI_46" + _C + ".急性心筋梗塞の診断で入院した患者数 " + \
            "FROM " + \
	            "CI_46" + _C + " " + \
            "WHERE " + \
	            "NOT CI_46" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_46" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_46" + _C + ".年度, CI_46" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_46 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_46" in wb.sheetnames:
        wb.remove(wb["CI_46"])
    wb.create_sheet("CI_46")
    sheet = wb["CI_46"]    
    ...
    #急性心筋梗塞患者におけるアスピリン_退院時投与
    temp急性心筋梗塞患者におけるアスピリン退院時投与率 = X_01.excuteSQL(getSQL_CI_46_急性心筋梗塞患者におけるアスピリン_退院時投与(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "急性心筋梗塞患者におけるアスピリン_退院時投与"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数_ラベル"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "急性心筋梗塞の診断で入院した患者数_比較用"

    if temp急性心筋梗塞患者におけるアスピリン退院時投与率 is None:
        print("エラー: 急性心筋梗塞患者におけるアスピリン退院時投与率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp急性心筋梗塞患者におけるアスピリン退院時投与率:
        ...
        #分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数
        ...
        #分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数_ラベル
        if tempRow[3] == -1:
        ...
        #分母のうち退院時にアスピリンもしくはクロピドグレルが投与された患者数_比較用
