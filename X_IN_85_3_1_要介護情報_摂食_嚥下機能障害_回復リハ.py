...
# 2024年度にCI番号変更
# CI_141　→　CI_85-3_1
# ただし、DBのテーブルは「CI_141」のまま
def getSQL_CI_85_3_1_要介護情報_摂食_嚥下機能障害_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_141" + _C + ".年度 ,  " + \
	            "CI_141" + _C + ".月, " + \
                "CI_141" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_141" + _C + " " + \
            "WHERE " + \
	            "CI_141" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_141" + _C + ".年度, CI_141" + _C + ".月 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_85_3_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_85_3_1" in wb.sheetnames:
        wb.remove(wb["CI_85_3_1"])
    wb.create_sheet("CI_85_3_1")
    sheet = wb["CI_85_3_1"]    

    #要介護情報_摂食_嚥下機能障害_回復リハ
    temp要介護情報_摂食_嚥下機能障害_回復リハ = X_01.excuteSQL(getSQL_CI_85_3_1_要介護情報_摂食_嚥下機能障害_回復リハ(HOSPITAL_ID, _C))

    if temp要介護情報_摂食_嚥下機能障害_回復リハ is None:
        print("エラー: 要介護情報_摂食_嚥下機能障害_回復リハの取得に失敗しました。")
        return -1
    ...
    for a in temp要介護情報_摂食_嚥下機能障害_回復リハ:
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "要介護情報_摂食_嚥下機能障害_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_85_3_1 終了")
