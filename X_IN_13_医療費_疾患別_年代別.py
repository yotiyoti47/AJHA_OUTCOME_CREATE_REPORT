...
def getSQL_CI_13_医療費_疾患別_年代別(disease_ID, age_ID, _C):
    return  "SELECT  " + \
	            "CI_13" + _C + ".年度 ,  " + \
	            "CI_13" + _C + ".期,  " + \
                "CI_13" + _C + ".入院費用, " + \
            ...
            "FROM " + \
	            "CI_13" + _C + " " + \
            "WHERE " + \
	            "NOT CI_13" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_13" + _C + ".Disease_DISEASE_ID = " + str(disease_ID) + " AND " + \
                "CI_13" + _C + ".Age_AGE_ID = " + str(age_ID) +  \
            " ORDER BY CI_13" + _C + ".年度, CI_13" + _C + ".期, CI_13" + _C + ".Disease_DISEASE_ID, CI_13" + _C + ".Age_AGE_ID " 
...
def getRepAgeData(wb, is_acute):
    ...
    # 既存シートが存在する場合は削除
    if "CI_13" in wb.sheetnames:
        wb.remove(wb["CI_13"])
    wb.create_sheet("CI_13")
    sheet = wb["CI_13"]    
    ...
    for tempDisease in CONST.MASTA_DISEASE:
        ...
        for tempAge in CONST.MASTA_AGE:
            ...
            temp医療費 = X_01.excuteSQL(getSQL_CI_13_医療費_疾患別_年代別(tempDisease[0], tempAge[0], _C))
            ...
            # 4行名に重症度、年代別、性別
            sheet.cell(4, 1 + (colCnt - 1) * 12).value = tempAge[1]
