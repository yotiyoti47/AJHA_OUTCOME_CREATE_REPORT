...
# 2024年度にCI番号変更
# CI_131　→　CI_83
# ただし、DBのテーブルは「CI_131」のまま
def getSQL_CI_83_退院前訪問指導料の算定率_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_131" + _C + ".年度 ,  " + \
	            "CI_131" + _C + ".月, " + \
                "CI_131" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_131" + _C + " " + \
            "WHERE " + \
	            "CI_131" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_131" + _C + ".年度, CI_131" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_83 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_83" in wb.sheetnames:
        wb.remove(wb["CI_83"])
    wb.create_sheet("CI_83")
    sheet = wb["CI_83"]    

    #退院前訪問指導料の算定率_療養
    temp退院前訪問指導料の算定率_療養 = X_01.excuteSQL(getSQL_CI_83_退院前訪問指導料の算定率_療養(HOSPITAL_ID, _C))

    if temp退院前訪問指導料の算定率_療養 is None:
        print("エラー: 退院前訪問指導料の算定率_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "退院前訪問指導料の算定率_療養"
    # 3行目に疾患名
