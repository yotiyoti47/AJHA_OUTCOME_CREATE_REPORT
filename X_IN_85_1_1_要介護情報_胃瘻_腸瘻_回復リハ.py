...
# 2024年度にCI番号変更
# CI_135　→　CI_85-1_1
# ただし、DBのテーブルは「CI_135」のまま
def getSQL_CI_85_1_1_要介護情報_胃瘻_腸瘻_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_135" + _C + ".年度 ,  " + \
	            "CI_135" + _C + ".月, " + \
                "CI_135" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_135" + _C + " " + \
            "WHERE " + \
	            "CI_135" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_135" + _C + ".年度, CI_135" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_85_1_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_85_1_1" in wb.sheetnames:
        wb.remove(wb["CI_85_1_1"])
    wb.create_sheet("CI_85_1_1")
    sheet = wb["CI_85_1_1"]    

    #要介護情報_胃瘻_腸瘻_回復リハ
    temp要介護情報_胃瘻_腸瘻_回復リハ = X_01.excuteSQL(getSQL_CI_85_1_1_要介護情報_胃瘻_腸瘻_回復リハ(HOSPITAL_ID, _C))

    if temp要介護情報_胃瘻_腸瘻_回復リハ is None:
        print("エラー: 要介護情報_胃瘻_腸瘻_回復リハの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "要介護情報_胃瘻_腸瘻_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_85_1_1 終了")
