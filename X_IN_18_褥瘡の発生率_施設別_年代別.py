...
def getSQL_CI_18_褥瘡の発生率_施設別_年代別(HOSPITAL_ID, Age_ID, _C):
    return  "SELECT  " + \
	            "CI_18" + _C + ".年度 ,  " + \
	            "CI_18" + _C + ".期,  " + \
                "CI_18" + _C + ".年代別褥瘡の発生率, " + \
            ...
            "FROM " + \
	            "CI_18" + _C + " " + \
            "WHERE " + \
	            "NOT CI_18" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_18" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_18" + _C + ".Age_AGE_ID = " + str(Age_ID) +  \
            " ORDER BY CI_18" + _C + ".年度, CI_18" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(HOSPITAL_NAME + " CI_18 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_18" in wb.sheetnames:
        wb.remove(wb["CI_18"])
    wb.create_sheet("CI_18")
    sheet = wb["CI_18"]    
    ...
    for tempAge in CONST.MASTA_AGE:
        ...
        temp褥瘡の発生率 = X_01.excuteSQL(getSQL_CI_18_褥瘡の発生率_施設別_年代別(HOSPITAL_ID, tempAge[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "d2（真皮までの損傷）以上の褥瘡発生率_年代別"
        # 3行目に疾患名
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "年代別褥瘡の発生率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "年代別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "年代別褥瘡の発生率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "年代別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "年代別褥瘡の発生率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "年代別症例数_比較用"

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp褥瘡の発生率:
...
def getRepAgeData_old(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    ...
    for tempAge in CONST.MASTA_AGE:
        ...
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "d2（真皮までの損傷）以上の褥瘡発生率"
        # 3行目に疾患名
        sheet.cell(3, 1 + (colCnt - 1) * roopColCnt).value = tempAge[1]
        # 4行名に重症度、年代別、性別
        sheet.cell(4, 1 + (colCnt - 1) * roopColCnt).value = ""
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt).value = "年度"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 1).value = "期"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "褥瘡（d2（真皮までの損傷）以上の褥瘡）の発生率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "年代別退院患者の在院日数合計"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "疾患別患者数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "褥瘡（d2（真皮までの損傷）以上の褥瘡）の発生率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "年代別退院患者の在院日数合計_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "疾患別患者数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "褥瘡（d2（真皮までの損傷）以上の褥瘡）の発生率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "年代別退院患者の在院日数合計_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "疾患別患者数_比較用"

        if temp褥瘡の発生率 is None:
            print("エラー: 褥瘡の発生率の取得に失敗しました。")
            return -1
