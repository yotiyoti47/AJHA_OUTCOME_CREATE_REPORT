...
def getSQL_CI_30_虫垂切除術患者に対する腹腔鏡下手術施行率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_30" + _C + ".年度 ,  " + \
	            "CI_30" + _C + ".期,  " + \
                "CI_30" + _C + ".虫垂切除術患者に対する腹腔鏡下手術施行率, " + \
                "CI_30" + _C + ".うち_腹腔鏡下手術施行症例数, " + \
                "CI_30" + _C + ".虫垂炎が医資最病名であり_虫垂炎切除術を施行された退院症例数 " + \
            "FROM " + \
	            "CI_30" + _C + " " + \
            "WHERE " + \
	            "NOT CI_30" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_30" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_30" + _C + ".年度, CI_30" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_30 開始")
    if is_acute == "急性期":
        _C = ""
    else:
        _C = "_C"
    ...
    # 既存シートが存在する場合は削除
    if "CI_30" in wb.sheetnames:
        wb.remove(wb["CI_30"])
    wb.create_sheet("CI_30")
    sheet = wb["CI_30"]    
    ...
    temp虫垂切除術患者に対する腹腔鏡下手術施行率 = X_01.excuteSQL(getSQL_CI_30_虫垂切除術患者に対する腹腔鏡下手術施行率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "虫垂切除術患者に対する腹腔鏡下手術施行率"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "虫垂切除術患者に対する腹腔鏡下手術施行率"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "うち_腹腔鏡下手術施行症例数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "虫垂炎が医資最病名であり_虫垂炎切除術を施行された退院症例数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "虫垂切除術患者に対する腹腔鏡下手術施行率_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "うち_腹腔鏡下手術施行症例数_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "虫垂炎が医資最病名であり_虫垂炎切除術を施行された退院症例数_ラベル"
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "虫垂炎が医資最病名であり_虫垂炎切除術を施行された退院症例数_比較用"

    if temp虫垂切除術患者に対する腹腔鏡下手術施行率 is None:
        print("エラー: 虫垂切除術患者に対する腹腔鏡下手術施行率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp虫垂切除術患者に対する腹腔鏡下手術施行率:
        ...
        #虫垂炎が医資最病名であり_虫垂炎切除術を施行された退院症例数
        ...
        #虫垂炎が医資最病名であり_虫垂炎切除術を施行された退院症例数_ラベル
        if tempRow[4] == -1:
