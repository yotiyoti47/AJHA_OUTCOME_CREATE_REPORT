...
def getSQL_CI_59_転倒転落発生率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_59" + _C + ".年度 ,  " + \
	            "CI_59" + _C + ".期,  " + \
                "CI_59" + _C + ".転倒転落発生率, " + \
                "CI_59" + _C + ".転倒転落の発生件数, " + \
                "CI_59" + _C + ".退院患者の在院日数の総和 " + \
            "FROM " + \
	            "CI_59" + _C + " " + \
            "WHERE " + \
	            "NOT CI_59" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_59" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_59" + _C + ".年度, CI_59" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_59 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_59" in wb.sheetnames:
        wb.remove(wb["CI_59"])
    wb.create_sheet("CI_59")
    sheet = wb["CI_59"]    
    ...
    #手転倒転落発生率
    temp転倒転落発生率 = X_01.excuteSQL(getSQL_CI_59_転倒転落発生率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "転倒転落発生率"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "転倒転落発生率"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "転倒転落の発生件数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "退院患者の在院日数の総和"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "転倒転落発生率_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "転倒転落の発生件数_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "退院患者の在院日数の総和_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "転倒転落発生率_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "転倒転落の発生件数_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "退院患者の在院日数の総和_比較用"

    if temp転倒転落発生率 is None:
        print("エラー: 転倒転落発生率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp転倒転落発生率:
