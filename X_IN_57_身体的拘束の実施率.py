...
def getSQL_CI_57_身体的拘束の実施率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_57" + _C + ".年度 ,  " + \
	            "CI_57" + _C + ".期,  " + \
                "CI_57" + _C + ".身体的拘束の実施率, " + \
                "CI_57" + _C + ".身体的拘束日数の総和, " + \
                "CI_57" + _C + ".退院患者の在院日数の総和 " + \
            "FROM " + \
	            "CI_57" + _C + " " + \
            "WHERE " + \
	            "NOT CI_57" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_57" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_57" + _C + ".年度, CI_57" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_57 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_57" in wb.sheetnames:
        wb.remove(wb["CI_57"])
    wb.create_sheet("CI_57")
    sheet = wb["CI_57"]    
    ...
    #血液培養の実施_2セット実施率
    temp身体的拘束の実施率 = X_01.excuteSQL(getSQL_CI_57_身体的拘束の実施率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "身体的拘束の実施率"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "身体的拘束の実施率"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "身体的拘束日数の総和"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "退院患者の在院日数の総和"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "身体的拘束の実施率_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "身体的拘束日数の総和_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "退院患者の在院日数の総和_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "身体的拘束の実施率_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "身体的拘束日数の総和_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "退院患者の在院日数の総和_比較用"

    if temp身体的拘束の実施率 is None:
        print("エラー: 身体的拘束の実施率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp身体的拘束の実施率:
