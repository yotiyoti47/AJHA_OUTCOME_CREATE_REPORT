...
def getSQL_CI_21_急性心筋梗塞における主要手術_治療実施率_年代別(HOSPITAL_ID, Age_ID, _C):
    return  "SELECT  " + \
	            "CI_21" + _C + ".年度 ,  " + \
	            "CI_21" + _C + ".期,  " + \
                "CI_21" + _C + ".年代別実施率, " + \
            ...
            "FROM " + \
	            "CI_21" + _C + " " + \
            "WHERE " + \
	            "NOT CI_21" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_21" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
	            "CI_21" + _C + ".Age_AGE_ID = " + str(Age_ID) +  \
            " ORDER BY CI_21" + _C + ".年度, CI_21" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(HOSPITAL_NAME + " CI_21 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_21" in wb.sheetnames:
        wb.remove(wb["CI_21"])
    wb.create_sheet("CI_21")
    sheet = wb["CI_21"]    
    ...
    for tempAge in CONST.MASTA_AGE:
        ...
        temp年代別実施率 = X_01.excuteSQL(getSQL_CI_21_急性心筋梗塞における主要手術_治療実施率_年代別(HOSPITAL_ID, tempAge[0], _C))
        #print("疾患名 " + tempDisease[1])

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "急性心筋梗塞における主要手術_治療実施率_年代別"
        # 3行目に疾患名
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "年代別実施率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "年代別症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "年代別実施率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "年代別症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "年代別実施率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "年代別症例数_比較用"


        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp年代別実施率:
