...
# 2024年度にCI番号変更
# CI_119　→　CI_76_1
# ただし、DBのテーブルは「CI_119」のまま
def getSQL_CI_76_1_紹介率_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_119" + _C + ".年度 ,  " + \
	            "CI_119" + _C + ".月, " + \
                "CI_119" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_119" + _C + " " + \
            "WHERE " + \
	            "CI_119" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_119" + _C + ".年度, CI_119" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_76_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_76_1" in wb.sheetnames:
        wb.remove(wb["CI_76_1"])
    wb.create_sheet("CI_76_1")
    sheet = wb["CI_76_1"]    

    #紹介率_回復リハ
    temp紹介率_回復リハ = X_01.excuteSQL(getSQL_CI_76_1_紹介率_回復リハ(HOSPITAL_ID, _C))

    if temp紹介率_回復リハ is None:
        print("エラー: 紹介率_回復リハの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "紹介率_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_76_1 終了")
