...
# 2024年度にCI番号変更
# CI_134　→　CI_84_3
# ただし、DBのテーブルは「CI_134」のまま
def getSQL_CI_84_3_要介護度_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_134" + _C + ".年度 ,  " + \
	            "CI_134" + _C + ".月, " + \
                "CI_134" + _C + ".療養病床_主目的, " + \
                ...
                "CI_134" + _C + ".要介護1_2, " + \
                "CI_134" + _C + ".要介護3_5, " + \
                "CI_134" + _C + ".申請中_不明_未入力 " + \
            "FROM " + \
	            "CI_134" + _C + " " + \
            "WHERE " + \
	            "CI_134" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_134" + _C + ".年度, CI_134" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_84_3 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_84_3" in wb.sheetnames:
        wb.remove(wb["CI_84_3"])
    wb.create_sheet("CI_84_3")
    sheet = wb["CI_84_3"]    

    #_要介護度_療養
    temp要介護度_療養 = X_01.excuteSQL(getSQL_CI_84_3_要介護度_療養(HOSPITAL_ID, _C))

    if temp要介護度_療養 is None:
        print("エラー: 要介護度_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "要介護度_療養"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_84_3 終了")
