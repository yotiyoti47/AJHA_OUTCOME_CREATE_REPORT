...
# 2024年度にCI番号変更
# CI_132　→　CI_84_1
# ただし、DBのテーブルは「CI_132」のまま
def getSQL_CI_84_1_要介護度_回復リハ(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_132" + _C + ".年度 ,  " + \
	            "CI_132" + _C + ".月, " + \
                "CI_132" + _C + ".病床数, " + \
                ...
                "CI_132" + _C + ".要介護1_2, " + \
                "CI_132" + _C + ".要介護3_5, " + \
                "CI_132" + _C + ".申請中_不明_未入力 " + \
            "FROM " + \
	            "CI_132" + _C + " " + \
            "WHERE " + \
	            "CI_132" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_132" + _C + ".年度, CI_132" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_84_1 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_84_1" in wb.sheetnames:
        wb.remove(wb["CI_84_1"])
    wb.create_sheet("CI_84_1")
    sheet = wb["CI_84_1"]    

    #要介護度_回復リハ
    temp要介護度_回復リハ = X_01.excuteSQL(getSQL_CI_84_1_要介護度_回復リハ(HOSPITAL_ID, _C))

    if temp要介護度_回復リハ is None:
        print("エラー: 要介護度_回復リハの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "要介護度_回復リハ"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_84_1 終了")
