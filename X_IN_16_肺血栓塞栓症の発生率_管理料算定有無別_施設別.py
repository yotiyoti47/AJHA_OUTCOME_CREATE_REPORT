...
def getSQL_CI_16_肺血栓塞栓症の発生率_管理料算定有無別_施設別(HOSPITAL_ID, calculation_ID, _C):
    return  "SELECT  " + \
	            "CI_16" + _C + ".年度 ,  " + \
	            "CI_16" + _C + ".期,  " + \
                "CI_16" + _C + ".肺血栓塞栓症予防管理料算定有無別発生率, " + \
                "CI_16" + _C + ".肺血栓塞栓症予防管理料算定有無別手術実施症例数 " + \
            "FROM " + \
	            "CI_16" + _C + " " + \
            "WHERE " + \
	            "NOT CI_16" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_16" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " AND " + \
                "CI_16" + _C + ".Calculation_CALCULATION_ID = " + str(calculation_ID) + " " + \
            " ORDER BY CI_16" + _C + ".年度, CI_16" + _C + ".期 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(HOSPITAL_NAME + " CI_16 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_16" in wb.sheetnames:
        wb.remove(wb["CI_16"])
    wb.create_sheet("CI_16")
    sheet = wb["CI_16"]    
    ...
    #管理料算定有無マスタの数だけループ
    for tempCalc in CONST.MASTA_CALCULATION:

        #肺血栓塞栓症の発生率
        temp肺肺血栓塞栓症の発生率 = X_01.excuteSQL(getSQL_CI_16_肺血栓塞栓症の発生率_管理料算定有無別_施設別(HOSPITAL_ID, tempCalc[0], _C))

        # 1行目に病院名
        sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
        # 2行目に指標名
        sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "肺血栓塞栓症の発生率_管理料算定有無別_施設別"
        # 3行目に疾患名
        sheet.cell(3, 1 + (colCnt - 1) * roopColCnt).value = tempCalc[1]
        # 4行名に重症度、年代別、性別
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "肺血栓塞栓症予防管理料算定有無別発生率"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "肺血栓塞栓症予防管理料算定有無別手術実施症例数"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "肺血栓塞栓症予防管理料算定有無別発生率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "肺血栓塞栓症予防管理料算定有無別手術実施症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "肺血栓塞栓症予防管理料算定有無別発生率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "肺血栓塞栓症予防管理料算定有無別手術実施症例数_比較用"

        # 6行目からデータ入力
        rowCnt = 6
        for tempRow in temp肺肺血栓塞栓症の発生率:
...
def getRepAgeData_old(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    ...
    for tempCalc in CONST.MASTA_CALCULATION:
        ...
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "肺血栓塞栓症予防管理料算定有無別発生率_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "肺血栓塞栓症予防管理料算定有無別手術実施症例数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "疾患別患者数_ラベル"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "肺血栓塞栓症予防管理料算定有無別発生率_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "肺血栓塞栓症予防管理料算定有無別手術実施症例数_比較用"
        sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "疾患別患者数_比較用"

        if temp肺肺血栓塞栓症の発生率 is None:      
            print("エラー: 肺肺血栓塞栓症の発生率の取得に失敗しました。")
            return -1
