...
def getSQL_CI_32_帝王切開における輸血施行率(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_32" + _C + ".年度 ,  " + \
	            "CI_32" + _C + ".期,  " + \
                "CI_32" + _C + ".帝王切開における輸血施行率, " + \
                "CI_32" + _C + ".うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数, " + \
                "CI_32" + _C + ".帝王切開手術を施行された退院症例数 " + \
            "FROM " + \
	            "CI_32" + _C + " " + \
            "WHERE " + \
	            "NOT CI_32" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_32" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_32" + _C + ".年度, CI_32" + _C + ".期 " 

def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_32 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_32" in wb.sheetnames:
        wb.remove(wb["CI_32"])
    wb.create_sheet("CI_32")
    sheet = wb["CI_32"]    
    ...
    #帝王切開における輸血施行率
    temp帝王切開における輸血施行率 = X_01.excuteSQL(getSQL_CI_32_帝王切開における輸血施行率(HOSPITAL_ID, _C))

    # 1行目に病院名
    sheet.cell(1, 1 + (colCnt - 1) * roopColCnt).value = HOSPITAL_NAME
    # 2行目に指標名
    sheet.cell(2, 1 + (colCnt - 1) * roopColCnt).value = "帝王切開における輸血施行率"
    # 3行目に疾患名
    ...
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 2).value = "帝王切開における輸血施行率"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 3).value = "うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 4).value = "帝王切開手術を施行された退院症例数"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 5).value = "帝王切開における輸血施行率_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 6).value = "うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 7).value = "帝王切開手術を施行された退院症例数_ラベル"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 8).value = "帝王切開における輸血施行率_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 9).value = "うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数_比較用"
    sheet.cell(5, 1 + (colCnt - 1) * roopColCnt + 10).value = "帝王切開手術を施行された退院症例数_比較用"

    if temp帝王切開における輸血施行率 is None:
        print("エラー: 帝王切開における輸血施行率の取得に失敗しました。")
        return -1

    # 6行目からデータ入力
    rowCnt = 6
    for tempRow in temp帝王切開における輸血施行率:
        ...
        #うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数
        ...
        #うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数_ラベル
        if tempRow[3] == -1:
        ...
        #うち_術日から術翌日までの輸血_赤血球または全血輸血_施行症例数_比較用
