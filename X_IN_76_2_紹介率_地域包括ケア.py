...
# 2024年度にCI番号変更
# CI_120　→　CI_76_2
# ただし、DBのテーブルは「CI_120」のまま
def getSQL_CI_76_2_紹介率_地域包括ケア(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_120" + _C + ".年度 ,  " + \
	            "CI_120" + _C + ".月, " + \
                "CI_120" + _C + ".病床数, " + \
            ...
            "FROM " + \
	            "CI_120" + _C + " " + \
            "WHERE " + \
	            "CI_120" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_120" + _C + ".年度, CI_120" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_76_2 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_76_2" in wb.sheetnames:
        wb.remove(wb["CI_76_2"])
    wb.create_sheet("CI_76_2")
    sheet = wb["CI_76_2"]    

    #紹介率_地域包括ケア
    temp紹介率_地域包括ケア = X_01.excuteSQL(getSQL_CI_76_2_紹介率_地域包括ケア(HOSPITAL_ID, _C))

    if temp紹介率_地域包括ケア is None:
        print("エラー: 紹介率_地域包括ケアの取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "紹介率_地域包括ケア"
    # 3行目に疾患名
    ...
    print(" " + HOSPITAL_NAME + " CI_76_2 終了")
