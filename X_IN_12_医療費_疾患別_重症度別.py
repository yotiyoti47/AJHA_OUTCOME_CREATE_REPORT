...
def getSQL_CI_12_医療費_疾患別_重症度別(disease_ID, Severity_ID, _C):
    return  "SELECT  " + \
	            "CI_12" + _C + ".年度 ,  " + \
	            "CI_12" + _C + ".期,  " + \
                "CI_12" + _C + ".入院費用, " + \
            ...
            "FROM " + \
	            "CI_12" + _C + " " + \
            "WHERE " + \
	            "NOT CI_12" + _C + ".期 = 'TOTAL' AND " + \
	            "CI_12" + _C + ".Disease_DISEASE_ID = " + str(disease_ID) + " AND " + \
                "CI_12" + _C + ".Severity_SEVERITY_ID = " + str(Severity_ID) +  \
            " ORDER BY CI_12" + _C + ".年度, CI_12" + _C + ".期,  CI_12" + _C + ".Disease_DISEASE_ID, CI_12" + _C + ".Severity_SEVERITY_ID " 
...
def getRepAgeData(wb, is_acute):
    ...
    # 既存シートが存在する場合は削除
    if "CI_12" in wb.sheetnames:
        wb.remove(wb["CI_12"])
    wb.create_sheet("CI_12")
    sheet = wb["CI_12"]    
    ...
    for tempDisease in CONST.MASTA_DISEASE:
        ...
        for tempSev in sevList:
            ...
            #print(getSQL_CI_12_医療費_疾患別_重症度別(tempDisease[0], tempSev[0]))
            temp医療費 = X_01.excuteSQL(getSQL_CI_12_医療費_疾患別_重症度別(tempDisease[0], tempSev[0], _C))
            #print("疾患名 " + tempDisease[1])

            # 1行目に病院名
            sheet.cell(1, 1 + (colCnt - 1) * 12).value = "全体"
            # 2行目に指標名
            sheet.cell(2, 1 + (colCnt - 1) * 12).value = "医療費_疾患別_重症度別"
            # 3行目に疾患名
            sheet.cell(3, 1 + (colCnt - 1) * 12).value = tempDisease[1]
            # 4行名に重症度、年代別、性別
            sheet.cell(4, 1 + (colCnt - 1) * 12).value = tempSev[1]
