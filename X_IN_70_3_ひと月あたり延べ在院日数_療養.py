...
# 2024年度にCI番号変更
# CI_105　→　CI_70_3
# ただし、DBのテーブルは「CI_105」のまま
def getSQL_CI_70_3_ひと月あたり延べ在院日数_療養(HOSPITAL_ID, _C):
    return  "SELECT  " + \
	            "CI_105" + _C + ".年度 ,  " + \
	            "CI_105" + _C + ".月, " + \
                "CI_105" + _C + ".療養病床_主目的, " + \
            ...
            "FROM " + \
	            "CI_105" + _C + " " + \
            "WHERE " + \
	            "CI_105" + _C + ".Hospital_HOSPITAL_ID = " + str(HOSPITAL_ID) + " " + \
            " ORDER BY CI_105" + _C + ".年度, CI_105" + _C + ".月 " 


def getRepAgeData(wb, HOSPITAL_ID, HOSPITAL_NAME, is_acute):
    print(" " + HOSPITAL_NAME + " CI_70_3 開始")
    ...
    # 既存シートが存在する場合は削除
    if "CI_70_3" in wb.sheetnames:
        wb.remove(wb["CI_70_3"])
    wb.create_sheet("CI_70_3")
    sheet = wb["CI_70_3"]    

    #ひと月あたり症例数_療養
    tempひと月あたり延べ在院日数_療養 = X_01.excuteSQL(getSQL_CI_70_3_ひと月あたり延べ在院日数_療養(HOSPITAL_ID, _C))

    if tempひと月あたり延べ在院日数_療養 is None:
        print("エラー: ひと月あたり延べ在院日数_療養の取得に失敗しました。")
        return -1
    ...
    # 2行目に指標名
    sheet.cell(2, roopColCnt).value = "ひと月あたり延べ在院日数_療養"
    # 3行目に疾患名
    ...
    for year in yearList:
        for key in keyList:
            ...
            tmpCnt = 1
            #療養病床_主目的
            sheet.cell(rowCnt, roopColCnt + tmpCnt).value = tempRow[2]
            tmpCnt+=1
            #年度
            ...
            if tempRow[4] == -1 or tempRow[4] == -2:
                ...
                sheet.cell(rowCnt, roopColCnt + tmpCnt).value = tempRow[4] 
            tmpCnt+=1
    ...
    print(" " + HOSPITAL_NAME + " CI_70_3 終了")
